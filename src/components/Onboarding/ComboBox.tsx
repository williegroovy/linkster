...
export default function ComboBox({ listItems, selected }: { listItems: ListItems, selected?: ListItems }) {
   ...
   const addTrade = api.profiles.addTrade.useMutation(
      {
         onSuccess: () => {
            router.refresh();
         }
   ...
   const handleOnKeyDown = async (e: KeyboardEvent<HTMLInputElement>) => {
      if(e.key === 'Enter') {
         ...
         if(filteredListItems.length === 0) {
            ...
            await addTrade.mutateAsync({ id: tradeLineItem.id });
            setQuery('');
   ...
   const handleOnChange = async (newListItems: ListItems) => {
      ...
      if(remove && remove.length > 0 && remove[0]?.id) {
         await removeTrade.mutateAsync({ id: remove[0].id });
      }
   }
